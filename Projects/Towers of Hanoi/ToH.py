...
def get_input():
    ...
    # choice = input("Select Stack to move: "
    #   + str(choices)).format(choices=choices)
    while True:
        user_input = input("Select Stack to move: " + str(choices)
                           + "").format(choices=choices).upper()
...
while right_stack.get_size() != num_disks:
    ...
    print("Remaining Optimal Moves: " +
          str(num_optimal_moves - num_user_moves))
    print("\n...Current Stacks...")
    left_stack.print_items()
    middle_stack.print_items()
    right_stack.print_items()
    ...
    if num_user_moves == num_optimal_moves:
        ...
        print("\n\nYou completed the game in {0} moves, and the optimal number\
             of moves is {1}".format(num_user_moves, num_optimal_moves))
