  ...
  def explore(self):
    ...
    while current_room != 'Treasure Room':
      ...
      for connected_room, weight in node.edges.items():
        ...
        print("Enter {0} for {1} at {2} cost".format(key, connected_room, weight))

      valid_choices = [room[0] for room in node.edges.keys()]
      print("\nYou have accumulated: {0} cost".format(path_total))

      choice = input("Which room do you move to?").upper()
      if choice not in valid_choices:
        print("Invalid choice. Please select from these letters {0}. ".format(valid_choices))
      else:
        ...
        print("\n*** You have chosen: {0} ***\n".format(current_room))
    print("You made it to the treasure room with {0} cost.".format(path_total))
